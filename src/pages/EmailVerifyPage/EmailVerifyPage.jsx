...
import { useResetVerifyEmailMutation } from "@/redux/auth/api";
...
export default function EmailVerifyPage() {
  ...
  const [resetVerifyEmail, { isLoading, isError, error }] =
    useResetVerifyEmailMutation();

  const onSubmit = async (data) => {
    try {
      await resetVerifyEmail({ email: data?.email }).unwrap();
      ...
      <div className='container relative flex pt-20 flex-col items-center justify-center lg:px-0 lg:pt-0'>
        <div className='mx-auto flex w-full flex-col justify-center space-y-6 sm:w-[360px]'>
          <div className='flex flex-col space-y-2 text-center'>
            <h1 className='text-2xl font-bold'>Reset Verify Email</h1>
            <p>Please enter your email to receive a email to reset verify.</p>
          </div>
          <div className='grid gap-6'>
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <Button type='submit' className='w-full' disabled={isLoading}>
                {isLoading ? "Sending..." : "Send Verify Link"}
              </Button>
            </form>
          </div>
        </div>
